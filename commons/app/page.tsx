...
function HomeView({ role, displayName, assignedIds, pinnedIds, canEdit, navigate, notify, onAdd, onRemove, onTogglePin }: { role: Role; displayName: string; assignedIds: Set<string>; pinnedIds: Set<string>; canEdit: boolean; navigate: (view: View) => void; notify: (message: string) => void; onAdd: (app: Application) => void; onRemove: (app: Application) => void; onTogglePin: (app: Application) => void }) {
  ...
  return <>
    <section className="welcome"><div><p className="kicker">H!KINEX COMMONS · {role.toUpperCase()}</p><h1>{displayName ? `Welcome, ${displayName}.` : "Welcome."}<br /><em>Your people and work, together.</em></h1><p>{profile.title} · {profile.team}</p>{!displayName && <p>To refresh your name, sign out and continue with Microsoft again.</p>}</div><div className="welcome-stats"><div><strong>{visibleApps.length}</strong><span>Pinned apps</span></div></div></section>
