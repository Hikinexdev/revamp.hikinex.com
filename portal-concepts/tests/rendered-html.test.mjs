...
test("keeps role navigation, official departments and safe actions", async () => {
  ...
  assert.match(page, /navByRole|role === "Admin"|role === "Manager"/);
  assert.match(page, /setPage\("All Apps"\)|setView\("Apps"\)/);
  assert.match(page, /Screaming Frog/);
  assert.match(page, /disabled|illustrative|prototype/i);
});
