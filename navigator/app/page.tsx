...
function HomeView({ role, notify, navigate }: { role: Role; notify: (message: string) => void; navigate: (view: View) => void }) {
  ...
  return <>
    <section className="welcome"><div><p className="kicker">{role.toUpperCase()} HOME</p><h1>Welcome, {profile.user}.<br /><em>Let’s make today count.</em></h1><p>{profile.title} · {profile.team}</p></div><div className="welcome-stats"><div><strong>{role === "Admin" ? "62" : role === "Manager" ? "8" : "6"}</strong><span>{role === "Admin" ? "Employees" : role === "Manager" ? "Team members" : "My apps"}</span></div><div><strong>{role === "Employee" ? "2" : "3"}</strong><span>Updates</span></div></div></section>
...
export default function Portal() {
  ...
  const nav = useMemo(() => { const base: View[] = ["Home", "Apps", "Announcements", "Feed", "Groups", "People", "Jobs"]; if (role === "Manager") base.push("Team", "Requests"); if (role === "Admin") base.push("Team", "Requests", "Admin"); return base; }, [role]);
